...
function C(f, a, b, d) {
  f.v.pa ? t(f, a, b, d) : f.v.Ze()._OnMessageFromDOM({
    type: "event",
    component: f.Wa,
    handler: a,
    dispatchOpts: d || null,
...
window.Eg = class extends self.za {
  constructor(f, a) {
    super(f, a);
    this.ca = new Map;
    this.Fc = !0;
  ...
  window.Ca = class v {
    constructor(g) {
      ...
      this.B("runtime", "cordova-fetch-local-file", k =>
        this.ff(k));
      this.B("runtime", "create-job-worker", () => this.gf());
    ...
    async rd(g) {
      "preview" === this.C && (this.ma = document.createElement("div"),
        this.ma.className = "previewLoadingMessage", this.ma.textContent = g.Jg, document.body.appendChild(this.ma));
      ...
      g.sg ? this.K = g.sg : (k = location.origin, this.K = ("null" === k ? "file:///" : k) + location.pathname, k = this.K.lastIndexOf("/"), -1 !== k && (this.K =
        this.K.substr(0, k + 1)));
      if (g.Cg)
        for (const [c, e] of Object.entries(g.Cg)) this.pb[c] = URL.createObjectURL(e);
      k = new MessageChannel;
      this.na = k.port1;
      this.na.onmessage = c => this._OnMessageFromRuntime(c.data);
      window.c3_addPortMessageHandler && window.c3_addPortMessageHandler(c => this.tf(c));
      ...
      this.pa ? await this.af(g, k.port2) :
        await this.$e(g, k.port2)
    }
    qc(g) {
    ...
    async $e(g, k) {
      ...
      "preview" === this.C &&
        "object" !== typeof self.Dg.Fg ? (this.Db(), console.error("[C3 runtime] Failed to load JavaScript code used in events. Check all your JavaScript code has valid syntax."), alert("Failed to load JavaScript code used in events. Check all your JavaScript code has valid syntax.")) : (g = Object.assign(this.qd(g), {
          isInWorker: !1,
          messagePort: k,
          canvas: this.J,
...
!k[1]).map(k=>k[0])[0]}'. Check all your JavaScript code has valid syntax.`;
    ...
    hd(g, k, c, e, l) {
      ...
      const k = g.type;
      if ("event" === k) return this.mf(g);
      if ("result" === k) this.Af(g);
      else if ("runtime-ready" === k) this.Bf();
    ...
    Dc() {
      -1 === this.La && this.Ka.length && (this.La = requestAnimationFrame(this.qg))
    }
    nc() {
    ...
    async Pe() {
      const g = [];
      for (const [k, c] of Object.entries(this.ha)) g.push(this.Oe(k, c));
      await Promise.all(g)
    }
    async Oe(g, k) {
  ...
  f.Qa(class extends self.za {
    constructor(c) {
      ...
      c.B("canvas", "update-size", m =>
        this.Ef(m));
      c.B("runtime", "invoke-download", m => this.rf(m));
      c.B("runtime", "raster-svg-image", m => this.yf(m));
      c.B("runtime", "get-svg-image-size", m => this.pf(m));
      c.B("runtime", "set-target-orientation", m => this.Cf(m));
      c.B("runtime", "register-sw", () => this.zf());
      c.B("runtime", "post-to-debugger", m => this.vd(m));
      c.B("runtime", "go-to-script", m => this.vd(m));
      c.B("runtime", "before-start-ticking", () => this.ef());
      c.B("runtime", "debug-highlight", m => this.hf(m));
      c.B("runtime", "enable-device-orientation", () => this.Me());
      c.B("runtime", "enable-device-motion", () => this.Le());
      c.B("runtime", "add-stylesheet", m => this.df(m));
      c.B("runtime", "alert", m => this.uc(m));
      c.B("runtime", "hide-cordova-splash", () => this.qf());
    ...
    Zc() {
      ...
      window.addEventListener("wheel", e => this.vf(e));
      "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", e => {
        this.rc(e);
        this.Ua("pointerdown", e)
      }), this.v.pa && "undefined" !== typeof window.onpointerrawupdate && self === self.top ? (this.lb = new self.Be(() => this.Ve(),
        5), this.lb.Ed = !0, window.addEventListener("pointerrawupdate", e => this.wf(e))) : window.addEventListener("pointermove", e => this.Ua("pointermove", e)), window.addEventListener("pointerup", e => this.Ua("pointerup", e)), window.addEventListener("pointercancel", e => this.Ua("pointercancel", e))) : (window.addEventListener("mousedown", e => {
        this.rc(e);
    ...
    Me() {
      this.Bd || (this.Bd = !0, window.addEventListener("deviceorientation", c => this.kf(c)), window.addEventListener("deviceorientationabsolute", c => this.lf(c)))
    }
    Le() {
...
"use strict";
async function J(f) {
  if (f.kg) throw Error("already initialised");
  f.kg = !0;
...
self.xe = class {
  constructor(f) {
    ...
    this.ng = Math.min(navigator.hardwareConcurrency || 2, 16);
    this.Ic = null;
    this.Kd = [];
...
"use strict";
{
  ...
  self.da = class extends self.za {
    constructor(a) {
      ...
      this.Rc = "";
      this.xa = null;
      self.C3Audio_OnMicrophoneStream = (b, d) => this.uf(b, d);
      this.Hb = null;
      self.C3Audio_GetOutputStream = () => this.nf();
      self.C3Audio_DOMInterface = this;
      E(this, [
        ["create-audio-context", b => this.Re(b)],
        ["play", b => this.Hf(b)],
        ["stop", b => this.dg(b)],
        ["stop-all", () => this.eg()],
        ["set-paused", b => this.Xf(b)],
        ["set-volume", b => this.bg(b)],
        ["fade-volume", b => this.We(b)],
        ["set-master-volume", b => this.Vf(b)],
        ["set-muted", b => this.Wf(b)],
        ["set-silent", b => this.Zf(b)],
        ["set-looping", b => this.Uf(b)],
        ["set-playback-rate", b => this.Yf(b)],
        ["seek", b => this.Of(b)],
        ["preload", b => this.If(b)],
        ["unload", b =>
          this.hg(b)
        ...
        ["unload-all", () => this.ig()],
        ["set-suspended", b => this.$f(b)],
        ["add-effect", b => this.nd(b)],
        ["set-effect-param", b => this.Rf(b)],
        ["remove-effects", b => this.Kf(b)],
        ["tick", b => this.Df(b)],
        ["load-state", b => this.sf(b)]
      ])
    }
    async Re(a) {
    ...
    async yb(a, b, d, h, n) {
      for (const q of this.aa)
        if (q.Sa() === b) return await M(q), q;
      if (n) return null;
      h && (this.Uc || this.Hd) &&
        this.De();
      n = "audio/webm; codecs=opus" === d && !this.rg;
      h && n && this.Sf();
    ...
    Da() {
      ...
      0 === b.length && 0 === this.Fa.size && this.Lb && (this.v.Mf(this.Ud),
        this.Lb = !1)
    }
    ic(a, b, d) {
      t(this, "trigger", {
    ...
    async Hf(a) {
      ...
      try {
        this.ga = await this.pc(b, d, h, q, n), w ? (this.ga.vb(!0), this.ga.Ee(w.x, w.y, w.angle, w.innerAngle, w.outerAngle, w.outerGain), w.hasOwnProperty("uid") &&
          this.ga.Ge(w.uid)) : this.ga.vb(!1), this.ga.Play(u, x, A, y)
      } catch (B) {
    ...
    xd() {
      this.Nc || (this.Nc = !0, Promise.resolve().then(() => this.Ue()))
    }
    Ue() {
      const a = {};
    ...
    async sf(a) {
      ...
      await Promise.all(d);
      await Promise.all(a.playing.map(h => this.cf(h, b)));
      this.zb()
    }
    async cf(a, b) {
      if (3 !== b) {
...
self.me = class extends self.ad {
  ...
  vc(f) {
    this.F || this.ia || f.target !== this.Va || (this.G = this.Kc = !0, this.fa = -1, P(this), this.i.ic("ended", this.Z, this.Gb))
  }
  T() {
  ...
  ub() {
    !this.F || this.G || this.T() || (P(this), this.j = this.W().createBufferSource(),
...
"use strict";
{
  ...
  self.ge = class extends f {
    constructor(a, b, d, h, n, q, u) {
      ...
      this.l = this.o();
      this.a = this.o();
      this.a.gain.value = 1 - u / 2;
      this.b = this.o();
      this.b.gain.value = u / 2;
      this.u = this.f.createBiquadFilter();
      this.u.type = "allpass";
  ...
  self.ce = class extends f {
    ...
    cg(a, b, d) {
      d = 1.05 * d * b - b;
      const h = 0 > a ? -a : a;
      return (h < b ? h : b + d * self.da.tg(h - b, 1 / d)) * (0 > a ? -1 : 1)
    }
    S(a) {
      this.b.disconnect();
      this.b.connect(a);
...
self.Ca.Qa(class extends self.za {
  constructor(f) {
    super(f, "touch");
    D(this, "request-permission", a => Q(this, a))
  }
});
"use strict";
      ...
      ["navigate", a => {
        ...
        if ("back" === b) navigator.app && navigator.app.backHistory ? navigator.app.backHistory() : window.back();
          ...
          "windows-uwp" === a && "undefined" !== typeof Windows ? Windows.System.Launcher.launchUriAsync(new Windows.Foundation.Uri(b)) :
            self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(b, "_system") : "preview" === a ? window.open(b, "_blank") : this.Gg || (2 === d ? window.top.location = b : 1 === d ? window.parent.location = b : window.location = b)
